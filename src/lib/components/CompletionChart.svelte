<script>
  ...
  export let completionSeries = [50, 50];

  let chartDiv;
  let chartInstance;

  let dropdownOpen = false;
  const periods = ["Current Month", "Last 3 Months", "Last 12 Months", "All Data"];
...
<div class="flex flex-col w-fit items-center bg-gray-800 rounded-md border border-gray-700 ">
  <div>
    <h5 class="mt-5">{Math.round((completionSeries[0] / (completionSeries[0] + completionSeries[1])) * 100)}%</h5>
    <p class="text-slate-400 text-lg font-semibold mb-2">Completion ratio</p>
  </div>
  <div bind:this={chartDiv}></div>
  <Button class="text-sm font-medium text-gray-500 dark:text-blue-700 hover:text-gray-900 text-center inline-flex items-center dark:hover:text-white bg-transparent hover:bg-transparent dark:bg-transparent dark:hover:bg-transparent focus:ring-transparent dark:focus:ring-transparent py-0 mb-2">
    {activePeriod}
  </Button>
